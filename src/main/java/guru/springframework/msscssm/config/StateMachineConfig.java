...
public class StateMachineConfig extends StateMachineConfigurerAdapter<PaymentState, PaymentEvent> {
    ...
    public Guard<PaymentState, PaymentEvent> paymentIdGuard(){
        return ctx -> ctx.getMessageHeader(PaymentServiceImpl.PAYMENT_ID_HEADER) != null;
    }
    ...
    public Action<PaymentState, PaymentEvent> preAuthAction() {
        return context -> {
            ...
            boolean r = new Random().nextInt(10) < 8;
            System.out.println(r ? "Pre Auth Approved" : "Per Auth Declined! No Credit!!!!!!");

            PaymentEvent paymentEvent = r ? PaymentEvent.PRE_AUTH_APPROVED : PaymentEvent.PRE_AUTH_DECLINED;
    ...
    public Action<PaymentState, PaymentEvent> authAction() {
        return context -> {
            ...
            boolean r = new Random().nextInt(10) < 8;
            System.out.println(r ? "Auth Approved" : "Auth Declined! No Credit!!!!!!");

            PaymentEvent paymentEvent = r ? PaymentEvent.AUTH_APPROVED : PaymentEvent.AUTH_DECLINED;
            context.getStateMachine().sendEvent(MessageBuilder.withPayload(paymentEvent)
                    .setHeader(PaymentServiceImpl.PAYMENT_ID_HEADER, context.getMessageHeader(PaymentServiceImpl.PAYMENT_ID_HEADER))
                    .build());
